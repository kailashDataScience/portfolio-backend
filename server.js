...
// ── ACTIVE CONNECTIONS (ping-based) ──
const activeVisitors = new Map(); // visitorId -> lastSeen timestamp

// POST /api/ping — called every 30s by portfolio page
app.post('/api/ping', (req, res) => {
  const { visitorId, device, page } = req.body;
  if (!visitorId) return res.json({ active: activeVisitors.size });
  activeVisitors.set(visitorId, { lastSeen: Date.now(), device: device||'unknown', page: page||'/' });
  // Clean up visitors not seen in last 90 seconds
  const cutoff = Date.now() - 90000;
  for (const [id, v] of activeVisitors.entries()) {
    if (v.lastSeen < cutoff) activeVisitors.delete(id);
  }
  res.json({ active: activeVisitors.size });
});

// GET /api/active — admin only, returns count + breakdown
app.get('/api/active', requireAuth, (req, res) => {
  ...
  const byDevice = visitors.reduce((acc, v) => {
    acc[v.device] = (acc[v.device]||0) + 1; return acc;
  }, {});
  res.json({ count: visitors.length, byDevice, visitors });
});
